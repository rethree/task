...
import { complete, fail, Option, Parallel, Task } from '../lib';
...
test('parallel can be used as a nested task', async () => {
  const task = Task(f => f(42)).chain(() =>
    Parallel(fail(42), complete(9001)).resume(x => x['fault']['length'])
  );
